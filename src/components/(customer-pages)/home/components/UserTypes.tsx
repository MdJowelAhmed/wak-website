            ...
            <div className="container mx-auto px-4">
                <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-5">
                    {userTypes.map(({ id, icon: Icon, iconBg, title, subtitle, cta, href, btnStyle }) => (
                        <div
                            key={id}
                            className="flex flex-col items-center text-center lg:flex-row lg:items-center lg:text-left gap-3 lg:gap-4 bg-primary/5 border border-border hover:border-primary/60 rounded-2xl p-4 md:p-5 transition-all duration-300 group shadow-xs hover:shadow-md"
                        >
                            {/* Icon circle */}
