            ...
            <div className="container mx-auto px-4">
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-5">
                    {userTypes.map(({ id, icon: Icon, iconBg, title, subtitle, cta, href, btnStyle }) => (
                        <div
                            key={id}
                            className="flex items-center gap-4 bg-white/8 hover:bg-white/12 border border-white/10 hover:border-[#FF6700]/40 rounded-2xl p-4 md:p-5 transition-all duration-300 group"
                        >
                            {/* Icon circle */}
                            <div className={`${iconBg} w-14 h-14 md:w-16 md:h-16 rounded-full flex items-center justify-center shrink-0 shadow-lg group-hover:scale-105 transition-transform duration-300`}>
                            ...
                            <div className="flex flex-col gap-2 min-w-0">
                                <div>
                                    ...
                                    <p className="text-white/60 text-xs md:text-sm mt-0.5 leading-snug">{subtitle}</p>
                                </div>
                                <Link
                                    href={href}
                                    className={`inline-flex items-center justify-center px-4 py-1.5 rounded-lg text-xs md:text-sm font-semibold transition-all duration-200 cursor-pointer w-fit ${btnStyle}`}
                                >
                                    {cta}
                                </Link>
                            </div>
