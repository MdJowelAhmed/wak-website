...
export default function ProductCategoriesPage() {
    ...
    const { categories, loading } = useCategories({ type: 'product' });

    const featuredCategories = categories.filter(
        (c) => c.isFeatured === true || String(c.isFeatured).toLowerCase() === 'true' || (c as any).isFeatured === 1
    );

    const activeCategories = tab === 'all' ? categories : featuredCategories;
            ...
            <div className="container mx-auto max-w-7xl">
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
                    <div>
                        ...
                        <p className="text-muted-text text-sm sm:text-base">
                            Browse all available product categories or filter by featured categories.
                        </p>
                    </div>
                ...
                {loading ? (
                    ...
                    <div className="text-center py-16 bg-card/40 rounded-2xl border border-card-border text-accent font-medium">
                        {tab === 'all' ? 'No product categories found' : 'No featured product categories found'}
                    </div>
                )}
            </div>
        </div>
